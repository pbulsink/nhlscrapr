produce.hexshots <- function (input.data) {
    ...
    subdata <- dplyr::mutate(input.data, gamestate = 1*(home.skaters == 6 & away.skaters == 6) +
                      2*(home.skaters > away.skaters) + 3*(home.skaters < away.skaters) +
                      4*(home.skaters == 5 & away.skaters == 5)) %>%
                          dplyr::filter(home.G > 1,
                                  away.G > 1,
                                  gamestate > 0,
                                  gamestate < 5) %>% dplyr::left_join(scatter.grid$xymap) %>% dplyr::rename(hexblock = hexbin)

    ## Get game table time on ice.
    total_timeonice_proto <- subdata %>% dplyr::group_by (season, gcode, gamestate) %>% dplyr::summarize(TTOI = sum(event.length))
    ttoi_away <- total_timeonice_proto %>% dplyr::mutate(home = 0)
    ttoi_away$gamestate[ttoi_away$gamestate %in% 2:3] <- 5 - ttoi_away$gamestate[ttoi_away$gamestate %in% 2:3]
    ttoi <- dplyr::rbind_list(total_timeonice_proto %>% dplyr::mutate(home=1),
                        ttoi_away)

    toi.one <- dplyr::rbind_list(group_by(subdata, season, gcode, gamestate, h1) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = h1) %>% dplyr::mutate(home=1),
                          dplyr::group_by(subdata, season, gcode, gamestate, h2) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = h2) %>% dplyr::mutate(home=1),
                          dplyr::group_by(subdata, season, gcode, gamestate, h3) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = h3) %>% dplyr::mutate(home=1),
                          dplyr::group_by(subdata, season, gcode, gamestate, h4) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = h4) %>% dplyr::mutate(home=1),
                          dplyr::group_by(subdata, season, gcode, gamestate, h5) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = h5) %>% dplyr::mutate(home=1),
                          dplyr::group_by(subdata, season, gcode, gamestate, h6) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = h6) %>% dplyr::mutate(home=1),
                          dplyr::group_by(subdata, season, gcode, gamestate, home.G) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = home.G) %>% dplyr::mutate(home=1),

                          dplyr::group_by(subdata, season, gcode, gamestate, a1) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = a1) %>% dplyr::mutate(home=0),
                          dplyr::group_by(subdata, season, gcode, gamestate, a2) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = a2) %>% dplyr::mutate(home=0),
                          dplyr::group_by(subdata, season, gcode, gamestate, a3) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = a3) %>% dplyr::mutate(home=0),
                          dplyr::group_by(subdata, season, gcode, gamestate, a4) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = a4) %>% dplyr::mutate(home=0),
                          dplyr::group_by(subdata, season, gcode, gamestate, a5) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = a5) %>% dplyr::mutate(home=0),
                          dplyr::group_by(subdata, season, gcode, gamestate, a6) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = a6) %>% dplyr::mutate(home=0),
                          dplyr::group_by(subdata, season, gcode, gamestate, away.G) %>%
                          dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(ID = away.G) %>% dplyr::mutate(home=0))

    toi.final <- toi.one %>% dplyr::filter(ID > 1) %>% dplyr::group_by(season, gcode, gamestate, home, ID) %>%
        dplyr::summarize(TOI=sum(TOI))
    ...
    toi.final <- dplyr::left_join(toi.final, ttoi) %>% dplyr::mutate(TOI.off = TTOI - TOI)


    toi.team <- dplyr::rbind_list(
        dplyr::group_by(subdata, season, gcode, gamestate, awayteam) %>%
            dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(Team = awayteam) %>% dplyr::mutate(home=0),
        dplyr::group_by(subdata, season, gcode, gamestate, hometeam) %>%
            dplyr::summarize(TOI = sum(event.length)) %>% dplyr::ungroup %>% dplyr::rename(Team = hometeam) %>% dplyr::mutate(home=1))
    ...
    shotdata <- dplyr::filter(subdata, !is.na(xcoord),
                        etype %in% c("GOAL","MISS","SHOT","BLOCK")) ## %>% dplyr::mutate (hexblock = hexbin.quick (cbind(newyc, newxc), scatter.grid))
    ...
    home.events <- dplyr::filter(shotdata, ev.team==hometeam)
    away.events <- dplyr::filter(shotdata, ev.team==awayteam)

    reduced.shots <- dplyr::rbind_list(sub.bit(home.events, "h1") %>% dplyr::mutate (home=1, role="O"),
                                 sub.bit(home.events, "h2") %>% dplyr::mutate (home=1, role="O"),
                                 sub.bit(home.events, "h3") %>% dplyr::mutate (home=1, role="O"),
                                 sub.bit(home.events, "h4") %>% dplyr::mutate (home=1, role="O"),
                                 sub.bit(home.events, "h5") %>% dplyr::mutate (home=1, role="O"),
                                 sub.bit(home.events, "h6") %>% dplyr::mutate (home=1, role="O"),
                                 sub.bit(home.events, "home.G") %>% dplyr::mutate (home=1, role="OG"),

                                 sub.bit(home.events, "a1") %>% dplyr::mutate (home=0, role="D"),
                                 sub.bit(home.events, "a2") %>% dplyr::mutate (home=0, role="D"),
                                 sub.bit(home.events, "a3") %>% dplyr::mutate (home=0, role="D"),
                                 sub.bit(home.events, "a4") %>% dplyr::mutate (home=0, role="D"),
                                 sub.bit(home.events, "a5") %>% dplyr::mutate (home=0, role="D"),
                                 sub.bit(home.events, "a6") %>% dplyr::mutate (home=0, role="D"),
                                 sub.bit(home.events, "away.G") %>% dplyr::mutate (home=0, role="G"),

                                 sub.bit(home.events, "ev.player.1") %>% dplyr::mutate (home=1, role="S"),

                                 sub.bit(away.events, "h1") %>% dplyr::mutate (home=1, role="D"),
                                 sub.bit(away.events, "h2") %>% dplyr::mutate (home=1, role="D"),
                                 sub.bit(away.events, "h3") %>% dplyr::mutate (home=1, role="D"),
                                 sub.bit(away.events, "h4") %>% dplyr::mutate (home=1, role="D"),
                                 sub.bit(away.events, "h5") %>% dplyr::mutate (home=1, role="D"),
                                 sub.bit(away.events, "h6") %>% dplyr::mutate (home=1, role="D"),
                                 sub.bit(away.events, "home.G") %>% dplyr::mutate (home=1, role="G"),

                                 sub.bit(away.events, "a1") %>% dplyr::mutate (home=0, role="O"),
                                 sub.bit(away.events, "a2") %>% dplyr::mutate (home=0, role="O"),
                                 sub.bit(away.events, "a3") %>% dplyr::mutate (home=0, role="O"),
                                 sub.bit(away.events, "a4") %>% dplyr::mutate (home=0, role="O"),
                                 sub.bit(away.events, "a5") %>% dplyr::mutate (home=0, role="O"),
                                 sub.bit(away.events, "a6") %>% dplyr::mutate (home=0, role="O"),
                                 sub.bit(away.events, "away.G") %>% dplyr::mutate (home=0, role="OG"),

                                 sub.bit(away.events, "ev.player.1") %>% dplyr::mutate (home=0, role="S")

                                 ) %>% dplyr::filter (ID > 1) %>% dplyr::mutate (playerteam = ifelse (home == 1, hometeam, awayteam))
    ...
    reduced.shots.team <- dplyr::rbind_list(
        sub.bit.team(home.events, "hometeam") %>% dplyr::mutate (home=1, role="O"),
        sub.bit.team(home.events, "awayteam") %>% dplyr::mutate (home=0, role="D"),
        sub.bit.team(away.events, "hometeam") %>% dplyr::mutate (home=1, role="D"),
        sub.bit.team(away.events, "awayteam") %>% dplyr::mutate (home=0, role="O"))


    ## baseline measurements.
    baseline.counts <- dplyr::group_by(shotdata, season, gamestate, new.loc.section) %>%
        dplyr::summarize(BLOCK=sum(etype=="BLOCK"), SHOT=sum(etype=="SHOT"),
                   MISS=sum(etype=="MISS"), GOAL=sum(etype=="GOAL"))
    baseline.time <- dplyr::group_by(subdata, season, gamestate) %>% dplyr::summarize(TOI=sum(event.length))
    ...
    baseline.data <- dplyr::left_join (baseline.counts, baseline.time)
...
game.toi <- function (mega.file="nhlscrapr-probs.RData",
  ...
  graphics::plot(gameprops[,5], gameprops[,6], ty="n")
  graphics::text(gameprops[,5], gameprops[,6], paste(gameprops$awayteam, gameprops$hometeam, sep="/"))
