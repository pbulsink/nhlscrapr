...
process.pl.new <- function (pl.html) {
  ...
  # add approximate changes.
    ...
    for (kk in 1:(orig.rec-1)) {
        past <- stats::na.omit(unlist(playbyplay[kk,6:17]))
        future <- stats::na.omit(unlist(playbyplay[kk+1,6:17]))
...
integrate.old.pieces <- function (home.image, away.image,
  ...
  #Add changes.
        ...
        for (kk in 1:(orig.rec-1)) {
            past <- stats::na.omit(unlist(pl.table$playbyplay[kk,6:17-1]))
            future <- stats::na.omit(unlist(pl.table$playbyplay[kk+1,6:17-1]))
