...
ordinal.maker <- function (vec, cuts=stats::quantile(vec, c(0, 0.35, 0.7), na.rm=TRUE)) {
    apply(1*outer (vec, cuts, ">="), 1, sum, na.rm=TRUE)
}
ordinal.maker.zeroes <- function (vec, cuts=stats::quantile(vec[vec>0], c(0, 0.35, 0.7))) {
    r1 <- 0*vec
...
hexbin <- function (xycoords) {
    dplyr::left_join(dplyr::select(xycoords, newxc, newyc), scatter.grid$xymap) %>% dplyr::select(hexbin) %>% c
}

nhl.hexbin <- function (xycoords) {
    basic <- data.frame(hexbin=1:nrow(scatter.grid$grid))
    bincount <- data.frame(hexbin(xycoords)) %>% dplyr::group_by(hexbin) %>% dplyr::summarize(count=n())
    totes <- dplyr::left_join(basic, bincount)
...
nhl.zonebin.prime <- function (bin.counts, use.superblocks=FALSE) {
  #sum up over bins in each zone. Redistribute over bins.
    ...
    if (use.superblocks) {
        ...
        count <- dplyr::group_by (data.frame(hexbin=scatter.grid$grid$zone.section.bigblock,
                                      count=bin.counts), hexbin) %>% dplyr::summarize (count=sum(count))
    } else {
        basic <- data.frame(hexbin=1:max(scatter.grid$grid$zone.section))
        count <- dplyr::group_by (data.frame(hexbin=scatter.grid$grid$zone.section,
                                      count=bin.counts), hexbin) %>% dplyr::summarize (count=sum(count))
    }
    retval <- (dplyr::left_join (basic, count))$count
    return(retval)
...
rink.hexplot <- function (scatter.grid, sizes, colors, bordercolor=NA, ...) {
  ...
  #sizes is now ordinal: 0=none, 1=0.4, 2=0.8, 3=0.8 black border
    ...
    graphics::polygon(hex.coords, col=colors, border=bordercolor, lwd=2)
